...
if(isset($_POST["BtnVistaPrevia"])){
	...
	$sel1=mysql_query("SELECT dpt.idDepartamentos as idDepartamento, dpt.Nombre as NombreDep, ROUND(SUM(v.TotalVenta*$Porcentaje)) as Total, ROUND(SUM(v.Descuentos*$Porcentaje)) as Descuentos, SUM(v.Cantidad) as NumVentas FROM ventas v INNER JOIN productosventa pr ON v.Referencia = pr.Referencia 
	INNER JOIN prod_departamentos dpt ON dpt.idDepartamentos = pr.Departamento
	WHERE v.Fecha >= '$FechaIni' AND v.Fecha <= '$FechaFinal' 
...
if(isset($_POST["BtnVistaPrevia"])){
$reg=mysql_query("SELECT SUM(TotalVenta*$Porcentaje) as Total from ventas  WHERE (Fecha >= '$FechaIni' AND Fecha <= '$FechaFinal') AND Impuestos=0 
