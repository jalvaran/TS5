...
if(isset($_POST["BtnVistaPrevia"])){
	...
	$sel1=mysql_query("SELECT dpt.idDepartamentos as idDepartamento, dpt.Nombre as NombreDep, ROUND(SUM(v.TotalVenta*$Porcentaje)) as Total, ROUND(SUM(v.Descuentos*$Porcentaje)) as Descuentos, ROUND(SUM(v.Cantidad*$Porcentaje)) as NumVentas FROM ventas v INNER JOIN productosventa pr ON v.Referencia = pr.Referencia 
	INNER JOIN prod_departamentos dpt ON dpt.idDepartamentos = pr.Departamento
	WHERE v.Fecha >= '$FechaIni' AND v.Fecha <= '$FechaFinal' 
	...
	mysql_query("UPDATE ventas SET TotalVenta=(ROUND(TotalVenta*$Porcentaje)), Impuestos=(ROUND(Impuestos*$Porcentaje)), Descuentos= (ROUND(Descuentos*$Porcentaje)), Cantidad= (Cantidad*$Porcentaje)
	WHERE Fecha >= '$FechaIni' AND Fecha <= '$FechaFinal'",$con) or die("problemas al actualizar la tabla ventas".mysql_error());
	
	$sel1=mysql_query("SELECT dpt.idDepartamentos as idDepartamento, dpt.Nombre as NombreDep, ROUND(SUM(v.TotalVenta)) as Total, ROUND(SUM(v.Descuentos)) as Descuentos, ROUND(SUM(v.Cantidad)) as NumVentas FROM ventas v INNER JOIN productosventa pr ON v.Referencia = pr.Referencia 
	INNER JOIN prod_departamentos dpt ON dpt.idDepartamentos = pr.Departamento
	WHERE v.Fecha >= '$FechaIni' AND v.Fecha <= '$FechaFinal' 
	GROUP BY dpt.idDepartamentos",$con) or die("problemas con la consulta a join ventas ".mysql_error());
...
if(isset($_POST["BtnVistaPrevia"])){
	$sel1=mysql_query("SELECT Usuarios_idUsuarios, ROUND(SUM(TotalVenta*$Porcentaje))as Total, ROUND(SUM(Descuentos*$Porcentaje)) as Descuentos , ROUND(SUM(Impuestos*$Porcentaje)) as IVA, ROUND(SUM(Cantidad*$Porcentaje)) as NumPro FROM ventas WHERE Fecha >= '$FechaIni' AND Fecha <= '$FechaFinal' 
...
if(isset($_POST["BtnVistaPrevia"])){
	$sel1=mysql_query("SELECT TipoVenta, ROUND(SUM(TotalVenta*$Porcentaje)) as Total, ROUND(SUM(Descuentos*$Porcentaje)) as Descuentos , ROUND(SUM(Impuestos*$Porcentaje)) as IVA, ROUND(SUM(Cantidad*$Porcentaje)) as NumPro FROM ventas WHERE Fecha >= '$FechaIni' AND Fecha <= '$FechaFinal' 
...
if(isset($_POST["BtnVistaPrevia"])){
	$sel1=mysql_query("SELECT Fecha, ROUND(SUM(TotalVenta*$Porcentaje)) as Total, ROUND(SUM(Descuentos*$Porcentaje)) as Descuentos , ROUND(SUM(Impuestos*$Porcentaje)) as IVA FROM ventas WHERE Fecha >= '$FechaIni' AND Fecha <= '$FechaFinal' 
...
if(isset($_POST["BtnVistaPrevia"])){
	$sel1=mysql_query("SELECT ROUND(SUM(TotalVenta*$Porcentaje)) as Total, ROUND(SUM(Descuentos*$Porcentaje)) as Descuentos, ROUND(SUM(Cantidad*$Porcentaje)) as Cantidad, ROUND(SUM(Impuestos*$Porcentaje)) as IVA, MAX(idVentas) as MaxId, MIN(idVentas) as MinId  FROM ventas WHERE Fecha >= '$FechaIni' AND Fecha <= '$FechaFinal' 
...
<table border="1" cellpadding="3" cellspacing="5" align="center">
 ...
 <tr style= "background-color:#EFFBF2;color:black;">
  <td>FACTURAS</td><td>INI: $MinId</td>
  <td>FIN: $MaxId</td>
