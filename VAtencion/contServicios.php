					...
					?></h2>
                      ...
                      <tr>
                        <td>
                            ...
                            <?php 
								$neto=0;$tneto=0;
								$pa=mysql_query("SELECT * FROM act_pre_movimientos
												WHERE NumOrden='$idUser'") 
								or die(mysql_error());				
								while($row=mysql_fetch_array($pa)){
									//$oProducto=new Consultar_Producto($row['Prod_Referencia']);
									$neto=$row['Cantidad']*($row['PrecioVenta']+$row['FichasModelos']+$row['ShowsModelos']+$row['Administrador']);
									$tneto=$tneto+$neto;
